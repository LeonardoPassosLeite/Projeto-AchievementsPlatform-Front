...
export class UserInsightsComponent implements OnInit, OnDestroy {
  ...
  loadAllData(): void {
    ...
    this.insightAchievementService.getAllAchievements(this.token).subscribe({
      next: (allData) => {
        this.allAchievements = allData;

        this.accountGameQuery.accountGames$.subscribe((accountGames) => {
          const gameStatsList = accountGames.map((accountGame) => accountGame.gameStats);
  ...
  loadTopPlayedGames(): void {
    console.log('Carregando os 3 jogos mais jogados...'); // Início do carregamento
    ...
    this.insightAchievementService.getAllAchievements(this.token).subscribe({
      next: (achievements) => {
        if (dataType === 'steam points') {
          ...
          const gameStatsList = this.accountGameQuery.getGameStatsList();
          if (year === 'Todos os Anos') {
